...
class AutorizacaoConvenio(BancoDadosTasy):
    ...
    def carteirinha(self, autorizacao: str) -> str:
        ...
        if not row[0]:
            raise Exception(["Excecao_Negocio", "Carteirinha não localizada."])
    ...
    def anexos(self, autorizacao: str) -> list:
        ...
        # Query SQL
        query = f"""
            SELECT DS_ARQUIVO  --UTL_URL.ESCAPE(DS_ARQUIVO) AS DS_ARQUIVO
        ...
        if len(rows) == 0:
            raise Exception(["Excecao_Negocio", "Anexos não localizados."])
    ...
    def solicitante(self, autorizacao: str) -> dict:
        ...
        row = self.cursor.fetchone()
        if row is None:
            raise Exception(["Excecao_Negocio", "Dados não localizados."])
    ...
    def procedimentos(self, autorizacao: str):
        ...
        rows = self.cursor.fetchall()
        if not rows:
            raise Exception(["Excecao_Negocio", "Procedimentos não localizados."])
