from config import (TASY_DB_USER, TASY_DB_PWD, TASY_DB_HOSTNAME, TASY_DB_SERVICENAME, RPA_DB_PWD, RPA_DB_USER,
                    RPA_DB_SERVER, RPA_DB_NAME)
...
"""
Consultas no banco de dados do Tasy a partir da sequência da autorização.
"""


def carteirinha(autorizacao: str) -> str:
    """
...
def anexos(autorizacao: str) -> list:
    """
...
def solicitante(autorizacao: str) -> dict:
    """
...
def procedimentos(autorizacao: str):
    """
...
def paciente(autorizacao: str) -> dict:
    """
...
def procedimento_interno(procedimento: str):
    """
    ...
    try:
        ...
        # Executa a consulta no banco de dados
        cursor.execute(query, CD_PROCEDIMENTO=str(procedimento))
...
def buscar_estagio(estabelecimento: str, tipo_estagio: str) -> str:
    """
    ...
    try:
        ...
        error_message = salvar_log_erro(error_sys=sys, task_error_message=mensagem_erro)
        raise ValueError(error_message)
