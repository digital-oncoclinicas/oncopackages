...
class ProtocoloConvenio(Tasy):
    ...
    def pesquisar_sequencia_protocolo(self, seq_protocolo: str) -> None:
        ...
        try:
            ...
            if not self.bot.element_wait_displayed(xpath="//input[@name='NR_SEQ_PROTOCOLO']", tentativas=20):
            ...
            if not self.bot.element_set_text(xpath="//input[@name='NR_SEQ_PROTOCOLO']", text=seq_protocolo, delay=1000):
            ...
            if self.bot.element_wait_displayed(xpath=f"//div[span='{seq_protocolo}']"):
                return
    ...
    def pesquisar_convenio(self, convenio: str) -> None:
        ...
        try:
            ...
            if not self.bot.element_wait_displayed(xpath="//input[@name='NR_SEQ_PROTOCOLO']", tentativas=10):
                # Clicar no ícone de Filtro que fica no canto superior esquerdo
                self.bot.element_click(xpath="//*[contains(@class,'filter-icon')]")
            ...
            # Aguarda a tela carregar
            if self.bot.element_wait_displayed(xpath="//*[contains(text(),'Adicionar')]", tentativas=30):
                return
    ...
    def adicionar_protocolo(self,
        ...
        try:
            ...
            if self.bot.element_wait_displayed(xpath=f"//span[text()='{protocolo}']", tentativas=40):
                return protocolo
    ...
    def mudar_status_protocolo(self, protocolo: str) -> None:
        ...
        try:
            ...
            # Verificar se salvou com sucesso
            if self.bot.element_wait_displayed(xpath=f"//span[text()='Definitivo']", tentativas=40):
                return
    ...
    def enviar_lote_faturamento_tiss(self, seq_protocolo: str) -> str:
        ...
        try:
            ...
            if not self.bot.element_wait_displayed(xpath=f"//div[span='{seq_protocolo}']"):
            ...
            # Clicar em "OK"
            if not self.bot.element_click("//button[contains(text(),'Ok')]"):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (OK) não localizado."])
    
            # Aguarda o download ser concluído
            arquivo = self.bot.esperar_conclusao_download(extensao_arquivo='.xml')
    ...
    def adicionar_anexo(self, dir_anexo: str) -> None:
        ...
        try:
            ...
            if not self.bot.find_element(xpath, By.XPATH, ensure_visible=True, ensure_clickable=True):
                ...
                if not self.bot.element_click("//div[div[div[div[contains(text(),'Anexo')]]]]"):
                    raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Menu (...) não localizado."])
    
                # Clica em anexo
                if not self.bot.element_click("//div[contains(text(),'Anexo')]"):
            ...
            # Clica em salvar
            if not self.bot.element_click("//div[div[div[span[contains(text(),'Salvar')]]]]"):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Salvar) não localizado."])
