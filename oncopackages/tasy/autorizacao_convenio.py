...
class AutorizacaoConvenio(Tasy):
    ...
    def adicionar_historico_autorizacao(self, tipo_historico: str, historico: str):
        ...
        try:
            ...
            mensagem_erro = self.bd_rpa.salvar_log_erro(mensagem_erro, self.bot)
        ...
        try:
            for item in lista_procedimentos:
                ...
                if not self.bot.find_element(f"//span[text() = '{procedimento}']", By.XPATH, waiting_time=3000):
                    ...
                    proc_interno = self.bot.bd_tasy.procedimento_interno(procedimento)
    
                    self.bot.adicionar_procedimento(
    ...
    def adicionar_autorizacao(
        ...
        try:
            # Clica no botão 'Adicionar'
            if not self.bot.element_click(xpath="//*[contains(text(), 'Adicionar')]", tentativas=30, delay=1000):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Adicionar) não localizado."])
            ...
            if guia_prestador != '':
                self.bot.element_set_text("//*[@name= 'CD_AUTORIZACAO_PREST']", guia_prestador)

            # Preenche o campo 'Gui operadora'
            if guia_operadora != '':
                self.bot.element_set_text("//*[@name= 'CD_AUTORIZACAO']", guia_operadora)

            # Preenche o campo 'Numero de protocolo'
            if numero_protocolo != '':
                self.bot.element_set_text("//*[@name= 'CD_SENHA_PROVISORIA']", numero_protocolo)

            # Preenche o campo 'Senha'
            if senha != '':
                self.bot.element_set_text("//*[@name= 'CD_SENHA']", senha)
    
            # Preenche o campo 'Data envio'
            if data_envio != '':
                self.bot.element_set_text("//*[@name= 'DT_ENVIO']", data_envio)

            # Preenche o campo 'Data retorno'
            if data_retorno != '':
                self.bot.element_set_text("//*[@name= 'DT_RETORNO']", data_retorno)

            # Preenche o campo 'Fim vigência'
            if data_fim_vigencia != '':
                self.bot.element_set_text("//*[@name= 'DT_FIM_VIGENCIA']", data_fim_vigencia)
    ...
    def gerar_relatorio_cate(self, autorizacao: str, cate: str) -> str:
        ...
        try:
            ...
            if not self.bot.element_click(f"//div[div[div[span[text() = '{autorizacao}']]]]"):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Autorização não localizada."])
    
            # Clica no menu "Relatórios -> Configurações"
            self.bot.element_click("//button[span[text()='Relatórios']]")
            if not self.bot.element_click("//div[text()='Configurações']"):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Menu 'Relatórios -> Configurações' não localizado."])
    
            # Preenche o campo 'Código'
            if not self.bot.element_set_text("//input[@name='CD_RELATORIO']", cate):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Tela de pesquisa não localizada."])
    
            # Clica em 'Filtrar'
            if not self.bot.element_click("//button[contains(text(),'Filtrar')]"):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Filtrar) não localizado."])
    
            # Clica em 'Visualizar'
            if not self.bot.element_click("//button[span[text()='Visualizar']]"):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Visualizar) não localizado."])
    
            # Esperar o download ser concluído
            arquivo_baixado = self.bot.esperar_conclusao_download(timeout=180000)
    
            # Clica em 'Cancelar'
            if not self.bot.element_click("//button[span[text()='Cancelar']]"):
