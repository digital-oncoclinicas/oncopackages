...
class EntradaUnicaPaciente(Tasy):
    ...
    def adicionar_atendimento(self,
        ...
        try:
            ...
            if not self.find_element("NR_ATENDIMENTO", By.NAME, waiting_time=5000, ensure_visible=True):
                # Clica em 'Adicionar'
                xpath = "//div[div[div[div[contains(text(),'Atendimentos')]]]]//*[contains(text(),'Adicionar')]"
                if not self.element_click(xpath=xpath, tentativas=6):
                    raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Adicionar) não localizado."])
                # Clica em 'Atendimento'
                if not self.element_click(xpath="//div[text()='Atendimento']", delay=250, tentativas=4):
                    raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Atendimento) não localizado."])

            # Pega o novo atendimento
            atendimento = ""
            for n in range(20):
                atendimento = self.find_element("NR_ATENDIMENTO", By.NAME).get_attribute("value")
                if atendimento != "":
                    break
                self.wait(500)
            if atendimento == "":
            ...
            for n in range(3):
                self.element_click(xpath="//div[input[@name='IE_TIPO_ATENDIMENTO']]", delay=1000, tentativas=4)
                self.element_click(xpath=f"//a[span[text()='{tipo_atendimento}']]", delay=250, tentativas=2)
                self.tab()
                tipo_atendimento_atual = self.element_get_text(xpath="//div[input[@name='IE_TIPO_ATENDIMENTO']]")
            ...
            self.element_click(xpath="//div[input[@name='IE_CLINICA'] and @tabindex='0']")
            if not self.element_click(xpath=f"//a[span[text()='{clinica_atendimento}']]", delay=250):
            ...
            self.element_click(xpath="//div[input[@name='CD_PROCEDENCIA']]")
            if not self.element_click(xpath=f"//a[span[contains(text(),'{procedencia}')]]", delay=250):
            ...
            self.element_left_click(xpath="//input[@name='CD_MEDICO_RESP']")
            for n in range(10):
                cd_medico_atendente_atual = self.element_get_value(xpath="//input[@name='CD_MEDICO_RESP']")
            ...
            tipo_convenio_atual = self.element_get_text(xpath="//div[input[@name='IE_TIPO_CONVENIO']]")
            if tipo_convenio != tipo_convenio_atual:
                self.element_click(xpath="//div[input[@name='IE_TIPO_CONVENIO']]")
                if not self.element_click(xpath=f"//a[span[text()='{tipo_convenio}']]", delay=250):
            ...
            if carater_atendimento != "":
                self.element_click(xpath="//div[input[@name='IE_CARATER_INTER_SUS']]")
                if not self.element_click(xpath=f"//a[span[text()='{carater_atendimento}']]", delay=250):
            ...
            if tipo_acidente != "":
                self.element_click(xpath="//div[input[@name='NR_SEQ_TIPO_ACIDENTE']]")
                if not self.element_click(xpath=f"//a[span[text()='{tipo_acidente}']]", delay=250):
            ...
            self.element_click(xpath="//div[input[@name='NR_SEQ_CLASSIFICACAO']]")
            if not self.element_click(xpath=f"//a[span[text()='{classificacao_atendimento}']]", delay=250):
            ...
            if tipo_atendimento_tiss != "":
                self.element_click(xpath="//div[input[@name='IE_TIPO_ATEND_TISS']]")
                if not self.element_click(xpath=f"//a[span[text()='{tipo_atendimento_tiss}']]", delay=250):
            ...
            if self.element_wait_displayed(xpath=f"//span[text()='{atendimento}']", tentativas=40):
                return atendimento
    ...
    def adicionar_convenio(self,
        ...
        try:
            ...
            if not self.find_element("CD_USUARIO_CONVENIO", By.NAME, waiting_time=5000, ensure_visible=True):
            ...
            convenio_atual = self.element_get_text(xpath="//div[input[@name='CD_CONVENIO']]//span")
            if convenio != "" and convenio != convenio_atual:
                self.element_click(xpath="//div[input[@name='NR_SEQ_CLASSIFICACAO']]", tentativas=4)
                if not self.element_click(xpath=f"//a[span[text()='{convenio}']]", delay=250, tentativas=4):
                    raise Exception([LOG_EX_NEGOCIO, mensagem_erro + f"Convênio ({convenio}) não localizado."])

            # Preenche o campo 'Categoria'
            categoria_atual = self.element_get_text(xpath="//div[input[@name='CD_CATEGORIA']]//span")
            if categoria != "" and categoria != categoria_atual:
                self.element_click(xpath="//div[input[@name='CD_CATEGORIA']]", tentativas=4)
                if not self.element_click(xpath=f"//a[span[text()='{categoria}']]", delay=250, tentativas=4):
                    raise Exception([LOG_EX_NEGOCIO, mensagem_erro + f"Categoria ({categoria}) não localizada."])

            # Preenche o campo 'Tipo de acomodação'
            tipo_acomodacao_atual = self.element_get_text(xpath="//div[input[@name='CD_TIPO_ACOMODACAO']]//span")
            if tipo_acomodacao != "" and tipo_acomodacao != tipo_acomodacao_atual:
                self.element_click(xpath="//div[input[@name='CD_TIPO_ACOMODACAO']]", tentativas=4)
                if not self.element_click(xpath=f"//a[span[text()='{tipo_acomodacao}']]", delay=250, tentativas=4):
            ...
            codigo_usuario_atual = self.element_get_value(xpath="//input[@name='CD_USUARIO_CONVENIO']")
            if codigo_usuario != "" and codigo_usuario != codigo_usuario_atual:
                self.element_set_text(xpath="//input[@name='CD_USUARIO_CONVENIO']", text=codigo_usuario, tentativas=4)
                self.tab()

            # Preenche o campo 'Complemento'
            complemento_atual = self.element_get_value(xpath="//input[@name='CD_COMPLEMENTO']")
            if complemento != "" and complemento != complemento_atual:
                self.element_set_text(xpath="//input[@name='CD_COMPLEMENTO']", text=complemento, tentativas=4)
                self.tab()

            # Preenche o campo 'Data de validade'
            data_validade_atual = self.element_get_value(xpath="//input[@name='DT_VALIDADE_CARTEIRA']")
            if data_validade != "" and data_validade != data_validade_atual:
                self.element_set_text(xpath="//input[@name='DT_VALIDADE_CARTEIRA']", text=data_validade, tentativas=4)
                self.tab()

            # Preenche o campo 'Plano/Produto'
            plano_produto_atual = self.element_get_text(xpath="//div[input[@name='CD_PLANO_CONVENIO']]//span")
            if plano_produto != "" and plano_produto != plano_produto_atual:
                self.element_click(xpath="//div[input[@name='CD_PLANO_CONVENIO']]", tentativas=4)
                if not self.element_click(xpath=f"//a[span[text()='{plano_produto}']]", delay=250, tentativas=4):
                    raise Exception([LOG_EX_NEGOCIO, mensagem_erro + f"Plano/Produto ({plano_produto}) não localizado."])

            # Preenche o campo 'Cobertura'
            cobertura_atual = self.element_get_text(xpath="//div[input[@name='NR_SEQ_COBERTURA']]//span")
            if cobertura != "" and cobertura != cobertura_atual:
                self.element_click(xpath="//div[input[@name='NR_SEQ_COBERTURA']]", tentativas=4)
                if not self.element_click(xpath=f"//a[span[text()='{cobertura}']]", delay=250, tentativas=4):
                    raise Exception([LOG_EX_NEGOCIO, mensagem_erro + f"Cobertura ({cobertura}) não localizada."])

            # Preenche o campo 'Tipo da guia'
            tipo_guia_atual = self.element_get_text(xpath="//div[input[@name='IE_TIPO_GUIA']]//span")
            if tipo_guia != "" and tipo_guia != tipo_guia_atual:
                self.element_click(xpath="//div[input[@name='IE_TIPO_GUIA']]", tentativas=4)
                if not self.element_click(xpath=f"//a[span[text()='{tipo_guia}']]", delay=250, tentativas=4):
                    raise Exception([LOG_EX_NEGOCIO, mensagem_erro + f"Tipo de guia ({tipo_guia}) não localizada."])

            # Preenche o campo 'Guia'
            guia_atual = self.element_get_value(xpath="//input[@name='NR_DOC_CONVENIO']")
            if guia != "" and guia != guia_atual:
                self.element_set_text(xpath="//input[@name='NR_DOC_CONVENIO']", text=guia, tentativas=4)
                self.tab()

            # Preenche o campo 'Senha'
            senha_atual = self.element_get_value(xpath="//input[@name='CD_SENHA']")
            if senha != "" and senha != senha_atual:
                self.element_set_text(xpath="//input[@name='CD_SENHA']", text=senha, tentativas=4)
                self.tab()

            # Preenche o campo 'Data de início de vigência'
            data_inicio_vigencia_atual = self.element_get_value(xpath="//input[@name='DT_INICIO_VIGENCIA']")
            if data_inicio_vigencia != "" and data_inicio_vigencia != data_inicio_vigencia_atual:
                self.element_set_text(xpath="//input[@name='DT_INICIO_VIGENCIA']", text=data_inicio_vigencia)
                self.tab()

            # Preenche o campo 'Data final da vigência'
            data_final_vigencia_atual = self.element_get_value(xpath="//input[@name='DT_FINAL_VIGENCIA']")
            if data_final_vigencia != "" and data_final_vigencia != data_final_vigencia_atual:
                self.element_set_text(xpath="//input[@name='DT_FINAL_VIGENCIA']", text=data_final_vigencia)
                self.tab()

            # Preenche o campo 'Observação'
            observacao_atual = self.element_get_value(xpath="//*[@name='DS_OBSERVACAO']")
            if observacao != "" and observacao != observacao_atual:
                self.element_set_text(xpath="//*[@name='DS_OBSERVACAO']", text=observacao, tentativas=4)
                self.tab()
            ...
            # Clica no botão 'Salvar'
            if not self.element_left_click(xpath="//div[div[div[span[text()='Salvar']]]]", tentativas=4):
    ...
    def adicionar_setores(self,
        ...
        try:
            ...
            if self.element_wait_displayed(xpath="//div[contains(text(),'Convênio')]", tentativas=1):
                if not self.element_click(xpath="//div[span[text()='Setores']]"):
                    # Clica nos '...' que tem no canto superior esquerdo para exibir as opções de aba
                    self.find_element("elipsisButton", By.ID).click()
                    # Clica em 'Setores'
                    if not self.element_click(xpath="//div[contains(text(),'Setores')]"):
            ...
            if not self.element_wait_displayed(xpath="//div[input[@name='CD_SETOR_ATENDIMENTO']]", tentativas=6):
                xpath = "//div[div[div[div[contains(text(),'Setores')]]]]//*[contains(text(),'Adicionar')]"
                if not self.element_click(xpath=xpath):
                    raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Adicionar) não localizado."])

            # Preenche o campo 'Setor de atendimento do paciente'
            setor_atendimento_atual = self.element_get_text(xpath="//div[input[@name='CD_SETOR_ATENDIMENTO']]//span")
            if setor_atendimento_paciente != setor_atendimento_atual:
                self.element_click(xpath="//div[input[@name='CD_SETOR_ATENDIMENTO']]")
                if not self.element_click(xpath=f"//a[span[text()='{setor_atendimento_paciente}']]", delay=250):
                ...
                if self.element_wait_displayed(xpath="//span[text()='Unidade Atendimento Disponível']", tentativas=4):
                    if not self.element_click(xpath=f"//div[div[div[span[text()='{unidade_basica}']]]]", tentativas=2):
                        raise Exception([LOG_EX_SISTEMA, mensagem_erro + f"Unidade básica ({unidade_basica}) não localizada."])
                    self.element_click(xpath=f"//button[span[contains(text(),'OK')]]")

                # Após selecionar o setor o tipo de acomodação e a Unidade básica são preenchidos automaticamente
                tipo_acomod_atual = ""
                for n in range(10):
                    tipo_acomod_atual = self.element_get_text(xpath="//div[input[@name='CD_TIPO_ACOMODACAO']]//span")
            ...
            if tipo_acomodacao != "" and tipo_acomodacao != tipo_acomod_atual:
                self.element_click(xpath="//div[input[@name='CD_TIPO_ACOMODACAO']]")
                if not self.element_click(xpath=f"//a[span[text()='{tipo_acomodacao}']]", delay=250):
            ...
            unidade_basica_atual = self.element_get_value(xpath="//input[@name='CD_UNIDADE_BASICA']")
            if unidade_basica != "" and unidade_basica != unidade_basica_atual:
                self.element_set_text(xpath="//input[@name='CD_UNIDADE_BASICA']", text=unidade_basica)
                self.tab()
    ...
    def gerar_alta_paciente(self, atendimento: str) -> None:
        ...
        try:
            ...
            if not self.element_right_click(xpath=xpath):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Atendimento não localizado."])

            # Clica em 'Atendimento -> Gerar alta paciente'
            self.element_click(xpath="//div[text()='Atendimento']")
            if not self.element_click(xpath="//div[text()='Gerar alta paciente']"):
            ...
            data_row_idx = self.find_element(selector=xpath, by=By.XPATH).get_attribute("data-row-idx")
    ...
    def retornar_tabela_atendimentos(self) -> None:
        ...
        try:
            ...
            if not self.element_click(xpath="//div[div[text()='Atendimento']]", delay=1000):
    ...
    def adicionar_diagnostico_medico(self, cid: str) -> None:
        ...
        try:
            ...
            if not self.element_click(xpath="//div[a[*[span[text()='Dados do atendimento']]]]"):
            ...
            if not self.element_click(xpath=xpath, delay=500):
                raise Exception([LOG_EX_SISTEMA, mensagem_erro + "Botão (Adicionar) não localizado."])

            # Clica no campo 'Medico' para fazer o Tasy pesquisar o médico anterior
            self.element_click(xpath="//input[@name='CD_MEDICO']")
            cd_medico_atual = ""
            for n in range(10):
                cd_medico_atual = self.element_get_value(xpath="//input[@name='CD_MEDICO']", tentativas=1)
                if cd_medico_atual != "":
                    break
                self.wait(500)
            if cd_medico_atual == "":
            ...
            self.element_set_text(xpath="//input[@name='CD_DOENCA']", text=cid)
            self.tab()
            descricao_cid = ""
            for n in range(10):
                descricao_cid = self.element_get_value(xpath="//input[@name='txDescription']")
                if descricao_cid != "":
                    break
                self.wait(500)
            if descricao_cid == "":
