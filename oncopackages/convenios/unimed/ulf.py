...
class SulAmerica(WebBotOp):
    ...
    def login(self, usuario: str, senha: str):
        ...
        try:
            ...
            self.navigate_to('https://remote.unimedlestefluminense.coop.br/connecta')
    
            # Tela cheia
            self.maximize_window()
    
            # Verifica se está na tela de login
            if self.find_element("txbUsuario", By.ID, waiting_time=5000):
                # Informa o usuário
                self.find_element("txbUsuario", By.ID).send_keys(usuario)
                # Informa o senha
                self.find_element("txbSenha", By.ID).send_keys(senha)
                # Clicar em Entrar
                self.find_element('btnEntrar', By.ID).click()
    
                # Verificar se o login foi realizado com sucesso
                if self.find_element("cphConteudo_ltvAtalhos_hplAtalho_0", By.ID, waiting_time=20000):
                    return
    
                # Checar mensagem erro
                if self.find_element("modalContentAlert", By.ID, waiting_time=1000):
                    erro = self.find_element("modalContentAlert", By.ID).get_attribute('innerText')
